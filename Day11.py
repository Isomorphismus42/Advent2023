...
image = open('inputs/input_day11', 'r').read().splitlines()
...
def part1():
    ...
    for j in range(len(universe[0])):
        ...
        for i in range(len(universe)):
            ...
            offset += 1
            for i in range(len(universe)):
                universe[i].insert(j, 0)
    universe = np.array(universe)
    galaxies = np.nonzero(universe)
    galaxies = set((map(lambda x, y: (x, y), galaxies[0], galaxies[1])))
    sum_shortest_paths = 0
    while len(galaxies) != 0:
        galaxy1 = galaxies.pop()
        for galaxy2 in galaxies:
            #  Manhattan-Distance
            sum_shortest_paths += abs(galaxy1[0] - galaxy2[0]) + abs(galaxy1[1] - galaxy2[1])
    ...
    while len(galaxies) != 0:
        ...
        for galaxy2 in galaxies:
            ...
            for empty_column in empty_columns:
                if min(galaxy1[1], galaxy2[1]) < empty_column < max(galaxy1[1], galaxy2[1]):
                    empty += 1
            sum_shortest_paths += abs(galaxy1[0] - galaxy2[0]) + abs(galaxy1[1] - galaxy2[1]) + (empty * 999999)
    return sum_shortest_paths
