...
the first sequence of differences that you generate will be 3 3 3 3 3. Note that this sequence has one fewer value
than the input sequence because at each step it considers two numbers from the input. Since these values aren't all
zero, repeat the process: the values differ by 0 at each step, so the next sequence is 0 0 0 0. This means you have
...
def create_sequences(line):
    ...
    while not all_zero:
        ...
        for k in range(n - i - 1):
            new_sequence.append(sequences[i][k + 1] - sequences[i][k])
        sequences.append(new_sequence)
        all_zero = all(j == 0 for j in new_sequence)
        i += 1
    return sequences
