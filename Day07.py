...
Island, making it hard to even get there. Normally, they use big machines to move the rocks and filter the sand,
but the machines have broken down because Desert Island recently stopped receiving the parts they need to fix the
machines.
...
Every hand is exactly one type. From strongest to weakest, they are:
    ...
    Five of a kind, where all five cards have the same label: AAAAA Four of a kind, where four cards have the same
    label and one card has a different label: AA8AA Full house, where three cards have the same label,
    and the remaining two cards share a different label: 23332 Three of a kind, where three cards have the same
    label, and the remaining two cards are each different from any other card in the hand: TTT98 Two pair,
    where two cards share one label, two other cards share a second label, and the remaining card has a third label:
    23432 One pair, where two cards share one label, and the other three cards have a different label from the pair
    and each other: A23A4 High card, where all cards' labels are distinct: 23456
...
So, the first step is to put the hands in order of strength:
    ...
    32T3K is the only one pair and the other hands are all a stronger type, so it gets rank 1. KK677 and KTJJT are
    both two pair. Their first cards both have the same label, but the second card of KK677 is stronger (K vs T),
    so KTJJT gets rank 2 and KK677 gets rank 3. T55J5 and QQQJA are both three of a kind. QQQJA has a stronger first
...
def evaluate_hand_part1(hand):
    cardvalue = {"A": 14, "K": 13, "Q": 12, "J": 11, "T": 10, "9": 9,
                  "8": 8, "7": 7, "6": 6, "5": 5, "4": 4, "3": 3, "2": 2}
    count_cards = dict()
    value_hand = 0
    for i, card in enumerate(hand[::-1]):
        value_hand += cardvalue[card] * (100 ** i)
        if card in count_cards:
            count_cards[card] += 1
        else:
            count_cards[card] = 1

    count_cards = sorted(count_cards.values(), reverse=True)
    if 5 == count_cards[0]:
        value_hand += 6 * (100 ** 5)
    elif 4 == count_cards[0]:
        value_hand += 5 * (100 ** 5)
    elif 3 == count_cards[0] and 2 == count_cards[1]:
        value_hand += 4 * (100 ** 5)
    elif 3 == count_cards[0]:
        value_hand += 3 * (100 ** 5)
    elif 2 == count_cards[0] and 2 == count_cards[1]:
        value_hand += 2 * (100 ** 5)
    elif 2 == count_cards[0]:
        value_hand += 1 * (100 ** 5)

    return value_hand


def evaluate_hand_part2(hand):
    cardvalue = {"A": 14, "K": 13, "Q": 12, "J": 1, "T": 10, "9": 9,
                  "8": 8, "7": 7, "6": 6, "5": 5, "4": 4, "3": 3, "2": 2}
    count_cards = dict()
    value_hand = 0
    for i, card in enumerate(hand[::-1]):
    ...
    if "J" in count_cards:
        ...
        joker = 0
    count_cards = sorted(count_cards.values(), reverse=True)
    highest = count_cards[0] + joker
    if 5 == highest:
        value_hand += 6 * (100 ** 5)
    elif 4 == highest:
        value_hand += 5 * (100 ** 5)
    elif 3 == highest and 2 == count_cards[1]:
        value_hand += 4 * (100 ** 5)
    elif 3 == highest:
        value_hand += 3 * (100 ** 5)
    elif 2 == highest and 2 == count_cards[1]:
        value_hand += 2 * (100 ** 5)
    elif 2 == highest:
        value_hand += 1 * (100 ** 5)
    return value_hand
...
def part1():
    cards_with_bids = list()
    with open('inputs/input_day7', 'r') as file:
        for line in file:
            line = line.split()
            cards_with_bids.append([evaluate_hand_part1(line[0]), int(line[1])])
    cards_with_bids.sort(key=lambda x: x[0])
    total_winnings = 0
    for i, hand in enumerate(cards_with_bids):
...
def part2():
    ...
    with open('inputs/input_day7', 'r') as file:
        for line in file:
            line = line.split()
            cards_with_bids.append([evaluate_hand_part2(line[0]), int(line[1])])
    cards_with_bids.sort(key=lambda x: x[0])
    total_winnings = 0
    for i, hand in enumerate(cards_with_bids):
        total_winnings += (i + 1) * hand[1]
