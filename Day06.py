...
So, because the first race lasts 7 milliseconds, you only have a few options:
    ...
    it will have traveled 0 millimeters by the end of the race. Hold the button for 1 millisecond at the start of the
    race. Then, the boat will travel at a speed of 1 millimeter per millisecond for 6 milliseconds, reaching a total
    distance traveled of 6 millimeters. Hold the button for 2 milliseconds, giving the boat a speed of 2 millimeters
    per millisecond. It will then get 5 milliseconds to move, reaching a total distance of 10 millimeters. Hold the
    button for 3 milliseconds. After its remaining 4 milliseconds of travel time, the boat will have gone 12
    millimeters. Hold the button for 4 milliseconds. After its remaining 3 milliseconds of travel time, the boat will
    have gone 12 millimeters. Hold the button for 5 milliseconds, causing the boat to travel a total of 10
    millimeters. Hold the button for 6 milliseconds, causing the boat to travel a total of 6 millimeters. Hold the
    button for 7 milliseconds. That's the entire duration of the race. You never let go of the button. The boat can't
    move until you let go of the button. Please make sure you let go of the button so the boat gets to move. 0
    millimeters.
...
def part1():
    with open('inputs/input_day6', 'r') as file:
        ...
        distance = list(map(int, file.readline().split()[1:]))
        races = list(map(lambda x, y: (x, y), times, distance))

    mult_number_of_ways = 1
    for time, distance in races:
        ...
        for t in range(time):
            if t * (time - t) > distance:
                won_races += 1
        mult_number_of_ways *= won_races
    return mult_number_of_ways


def part2():
    with open('inputs/input_day6', 'r') as file:
        time = int(file.readline().replace(" ", "").split(":")[1])
        distance = int(file.readline().replace(" ", "").split(":")[1])
    roots = np.roots([1, -time, distance])  # t^2 - t * time + distance = 0
    return int(np.floor(roots[0]) - np.ceil(roots[1]) + 1)
