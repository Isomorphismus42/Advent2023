...
So, if you were to look one additional map-width or map-height out from the edge of the example map above,
you would find that it keeps repeating:
...
def part1():
    ...
    for n in range(64):
        ...
        for position in queue:
            x, y = position
            if x > 0:
                if up_to_date_map[x - 1, y] == '.':
                    new_queue.add((x - 1, y))
            if x < up_to_date_map.shape[0] - 1:
                if up_to_date_map[x + 1, y] == '.':
                    new_queue.add((x + 1, y))
            if y > 0:
                if up_to_date_map[x, y - 1] == '.':
                    new_queue.add((x, y - 1))
            if y < up_to_date_map.shape[1] - 1:
                if up_to_date_map[x, y + 1] == '.':
                    new_queue.add((x, y + 1))
        queue = new_queue
    return len(queue)
...
def part2():
    ...
    for n in range(64):  # 26501365
        ...
        for position in queue:
            x, y = position
            mod_x = x % (up_to_date_map.shape[0])
            mod_y = y % (up_to_date_map.shape[1])
            if up_to_date_map[(x - 1) % up_to_date_map.shape[0], mod_y] == '.':
                new_queue.add((x - 1, y))
            if up_to_date_map[(x + 1) % up_to_date_map.shape[0], mod_y] == '.':
                new_queue.add((x + 1, y))
            if up_to_date_map[mod_x, (y - 1) % up_to_date_map.shape[1]] == '.':
                new_queue.add((x, y - 1))
            if up_to_date_map[mod_x, (y + 1) % up_to_date_map.shape[1]] == '.':
                new_queue.add((x, y + 1))
        queue_copy = queue.copy()
        queue = new_queue.difference(old_queue)
        old_queue = queue_copy
        if even:
            positions_odd += len(queue)
        else:
            positions_even += len(queue)
        even = not even
    # print(positions_odd)
    # for position in positions_odd:
    #     up_to_date_map[position] = 'O'
    # print(up_to_date_map)
    return positions_even if even else positions_odd
