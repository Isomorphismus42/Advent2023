...
def calculate_possibilities(springs, nonogramm):

    # Checke Abbruchbedingung keine Zahlen mehr übrig
    if not nonogramm:
        if '#' not in springs:
            return 1
        else:
    ...
    # Checke Abbruchbedingung keine Zahnräder mehr übrig, aber noch Zahlen vorhanden
    if springs == '':
        return 0

    if springs[0] == '.':
        return calculate_possibilities(springs[1:], nonogramm)
    else:
        possibilities = 0
        if springs[0] == '?':
            possibilities += calculate_possibilities(springs[1:], nonogramm)
        # Prüfe, ob Zahl passt, falls nicht, dann nicht lösbar
        if springs[:nonogramm[0]].replace('?', '#') == ('#' * nonogramm[0]):
            # Prüfe, ob letzte Zahl und am Ende des Strings
            if len(springs) == nonogramm[0]:
                if len(nonogramm) == 1:
                    return possibilities + 1
                else:
                    return possibilities
            # springs > nonogramm[0] und '.' oder '?' muss nun auf #-Kette folgen
            elif springs[nonogramm[0]] != '#':
                return possibilities + calculate_possibilities(springs[nonogramm[0] + 1:], nonogramm[1:])
            else:
                return possibilities
        else:
            return possibilities
...
def part1():
    ...
    with open('inputs/input_day12', 'r') as file:
        for line in file:
            springs, nonogram = line.split()
            nonogram = [int(i) for i in nonogram.split(',')]
            sum_possible_arrangements += calculate_possibilities(springs, tuple(nonogram))
    return sum_possible_arrangements
