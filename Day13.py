...
the first pattern's new horizontal line has 3 rows above it and the second pattern's new horizontal line has 1 row
...
def count_mirror_lines(pattern):
    n = len(pattern)
    for i, row in enumerate(pattern[:-1]):
        if i < int(n / 2):
            up = pattern[0:i+1][::-1]
            down = pattern[i + 1:2 * i + 2]
        else:
            up = pattern[i:(2 * i - n + 1):-1]
            down = pattern[i + 1:n]
        if up == down:
            return i + 1
    else:
...
def count_mirror_lines_part2(pattern):
    n = len(pattern)
    for i, row in enumerate(pattern[:-1]):
        if i < int(n / 2):
            up = pattern[0:i+1][::-1]
            ...
            up = pattern[i:(2 * i - n + 1):-1]
            down = pattern[i + 1:n]
        if np.sum(np.array(up) != np.array(down)) == 1:
            return i + 1
    else:
        return 0
...
def part1():
    patterns = open('inputs/input_day13', 'r').read().split("\n\n")
    sum_patterns = 0
    for pattern in patterns:
        pattern = pattern.split('\n')
        # rows
        sum_patterns += 100 * count_mirror_lines(pattern)
        pattern = list(zip(*pattern))
        # columns
        sum_patterns += count_mirror_lines(pattern)
    return sum_patterns
...
def part2():
    ...
    sum_patterns = 0
    for pattern in patterns:
        pattern = [list(s) for s in pattern.split('\n')]
        # rows
        sum_patterns += 100 * count_mirror_lines_part2(pattern)
        pattern = list(zip(*pattern))
        # columns
        sum_patterns += count_mirror_lines_part2(pattern)
    return sum_patterns
