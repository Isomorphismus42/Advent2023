...
------------------- Part Two -------------------- Just as you're about to report your findings to the Elf,
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11
    ...
    Card 1 has four matching numbers, so you win one copy each of the next four cards: cards 2, 3, 4, and 5. Your
    original card 2 has two matching numbers, so you win one copy each of cards 3 and 4. Your copy of card 2 also
    wins one copy each of cards 3 and 4. Your four instances of card 3 (one original and three copies) have two
    matching numbers, so you win four copies each of cards 4 and 5. Your eight instances of card 4 (one original and
    seven copies) have one matching number, so you win eight copies of card 5. Your fourteen instances of card 5 (one
    original and thirteen copies) have no matching numbers and win no more cards. Your one instance of card 6 (one
...
def part1():
    ...
    with open('inputs/input_day4', 'r') as file:
        for line in file:
            ...
            owned_numbers = {int(i) for i in owned_numbers.strip().replace("  ", " ").split(" ")}
            hits = len(owned_numbers.intersection(winning_numbers))
            sum_points += 2 ** (hits - 1) if hits > 0 else 0
    return sum_points


def part2():
    number_of_cards = {i:0 for i in range(1, 206)}
    card_nr = 1
    with open('inputs/input_day4', 'r') as file:
        for line in file:
            number_of_cards[card_nr] += 1
            line = line[10:-1].strip()
            winning_numbers, owned_numbers = line.split(' | ')
            winning_numbers = {int(i) for i in winning_numbers.replace("  ", " ").split(" ")}
            owned_numbers = {int(i) for i in owned_numbers.strip().replace("  ", " ").split(" ")}
            ...
            for i in range(1, hits + 1):
                number_of_cards[card_nr + i] += number_of_cards[card_nr]
            card_nr += 1
