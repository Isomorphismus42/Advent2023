...
further into the facility. At one point, you pass a button with a faint snout mark and the label "PUSH FOR HELP". No
...
order, filling any space made by removing the indicated lens. (If no lens in that box has the given label,
nothing happens.)
...
the beginning of the step so you can find it later. There are two possible situations:
    ...
    lens and put the new lens in its place, not moving any other lenses in the box. If there is not already a lens in
    the box with the same label, add the lens to the box immediately behind any lenses already in the box. Don't move
    any of the other lenses when you do this. If there aren't any lenses in the box, the new lens goes all the way to
...
def part1():
    initialization_sequence = open('inputs/input_day15', 'r').read().strip().split(',')
    sum_results = 0
    for steps in initialization_sequence:
        sum_results += calculate_hash(steps)
    return sum_results
