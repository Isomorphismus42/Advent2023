...
The pipes are arranged in a two-dimensional grid of tiles:
    ...
    | is a vertical pipe connecting north and south. - is a horizontal pipe connecting east and west. L is a
    90-degree bend connecting north and east. J is a 90-degree bend connecting north and west. 7 is a 90-degree bend
    connecting south and west. F is a 90-degree bend connecting south and east. . is ground; there is no pipe in this
    tile. S is the starting position of the animal; there is a pipe on this tile, but your sketch doesn't show what
    shape the pipe has.
...
field = open('inputs/input_day10', 'r').read().splitlines()
...
def find_circle():
    ...
    while True:
        ...
        pipe_type = field[i][j]
        if pipe_type == "|":
            if prev_position == (i - 1, j):
                position = (i + 1, j)
            else:
                position = (i - 1, j)
        elif pipe_type == "-":
            if prev_position == (i, j - 1):
                position = (i, j + 1)
            else:
                position = (i, j - 1)
        elif pipe_type == "L":
            ...
            if field[i][j + 1] in {"-", "7", "J"}:
                position = (i, j + 1)
                if field[i][j - 1] in {"-", "F", "L"}:
                    field[i] = field[i][:j] + "-" + field[i][j + 1:]
                elif field[i - 1][j] in {"|", "F", "7"}:
                    field[i] = field[i][:j] + "L" + field[i][j + 1:]
                elif field[i + 1][j] in {"|", "J", "L"}:
                    field[i] = field[i][:j] + "F" + field[i][j + 1:]
            elif field[i][j - 1] in {"-", "F", "L"}:
                position = (i, j - 1)
                if field[i - 1][j] in {"|", "F", "7"}:
                    field[i] = field[i][:j] + "J" + field[i][j + 1:]
                elif field[i + 1][j] in {"|", "J", "L"}:
                    field[i] = field[i][:j] + "7" + field[i][j + 1:]
            else:
                position = (i + 1, j)
                field[i] = field[i][:j] + "|" + field[i][j + 1:]
        if position == start:
...
def part1():
    return int(len(find_circle()) / 2)


def part2():
    circle = set(find_circle())
    m = len(field)
    n = len(field[0])

    parity_matrix = np.ones((m, n))
    for i in range(m):
        parity = False
        last = ''
        for j in range(n):
            if (i, j) in circle:
                edge = field[i][j]
                if edge == '|' or (edge == '7' and last == 'L') or (edge == 'J' and last == 'F'):
                    parity = not parity
                    last = ''
