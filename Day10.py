...
def find_circle():
    ...
    while True:
        ...
        if pipe_type == "|":
            ...
            if field[j][i + 1] in {"-", "7", "J"}:
                position = (i + 1, j)
            elif field[j][i - 1] in {"-", "F", "L"}:
                position = (i - 1, j)
            else:
                position = (i, j + 1)
        if position == start:
...
def part2():
    # Schritt 1: Orientierung herausfinden - welche Seite des Kreises zeigt nach innen?
    # Schritt 2: Kreis ablaufen - wenn angrenzendes "inneres Feld" nicht Teil von Kreis oder bereits gefunden, dann +1
    # und Breitensuche über angrenzende Felder, die nicht Teil Kreis und nicht gefunden.
